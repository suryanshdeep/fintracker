...
const DashBoardOverview = ({accounts, transactions }) => {
    ...
    //Filter the transaction for selected account
    const filteredTransactions = transactions.filter(
        (transaction) => transaction.accountId === selectedAccountId
    );
    // getting the recent 5 transactions from the account
    const recentTransactions = filteredTransactions
    .sort((a,b)=> new Date(b.date) - new Date(a.date)) // Sort by date descending
    .slice(0, 5);

    //Calculate the expense breakdown for the current month for the selected account
    ...
    const expensesByCategory = currentMonthExpenses.reduce((acc,transactions)=>{
        const category =transactions.category;
        if(!acc[category]){
            acc[category]=0;
        }
        acc[category] += transactions.amount;
        return acc;
    },{})
